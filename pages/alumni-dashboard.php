...
if ($ucsProfile === null) {
    ...
    if ($ucsProfileStatus === 'verified') {
        try {
            $ucsProfileStmt = $pdo->prepare(
            ...
            $ucsProfileStmt->execute([':student_id' => (int) $ucsStudent['id']]);
            $ucsProfile = $ucsProfileStmt->fetch() ?: null;
        } catch (PDOException $e) {
            $ucsProfile = null;
        }
        if ($ucsProfile !== null && $ucsProfile['verification_status'] === 'verified') {
            goto verified_dashboard;
        }
    }
