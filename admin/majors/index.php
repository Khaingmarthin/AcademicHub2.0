...
$pageTitle    = 'Majors';
$pageSubtitle = 'Manage majors and degree programmes.';
$activeNav    = 'majors';

$ucsFlash = $_SESSION['major_flash'] ?? null;
unset($_SESSION['major_flash']);
...
try {
    if ($ucsQuery !== '') {
        ...
        $ucsStmt = $pdo->prepare(
            "SELECT id, name, short_name, degree_name, description, status, created_at
             FROM majors
             WHERE name LIKE :query OR short_name LIKE :query OR degree_name LIKE :query
             ORDER BY name ASC"
        );
...
<div class="rounded-2xl bg-white shadow-sm ring-1 ring-gray-100">
    <div class="flex flex-col gap-4 border-b border-gray-100 p-6 lg:flex-row lg:items-center lg:justify-between">
        <div>
            <h2 class="text-base font-semibold text-gray-900">Major List</h2>
            <p class="mt-1 text-sm text-gray-500">Majors are used to group courses, classrooms and degree programmes.</p>
        </div>

        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
            <form method="get" action="<?php echo htmlspecialchars(ROOT_URL . '/admin/majors/index.php'); ?>" class="relative" role="search">
                ...
                <input type="search" name="q" value="<?php echo htmlspecialchars($ucsQuery); ?>" placeholder="Search majors…" aria-label="Search majors"
            ...
            <a href="<?php echo htmlspecialchars(ROOT_URL . '/admin/majors/create.php'); ?>" class="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition-colors duration-200 hover:bg-blue-700 focus:outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600">
    ...
    <?php if (empty($ucsMajors)): ?>
        <div class="p-10 text-center">
            ...
            <h3 class="mt-4 text-lg font-semibold text-gray-800">
                <?php echo $ucsQuery !== '' ? 'No matching majors' : 'No majors yet'; ?>
            </h3>
            <p class="mt-2 text-sm text-gray-500">
                <?php echo $ucsQuery !== '' ? 'Try a different search term.' : 'Add your first major to get started.'; ?>
            </p>
            <?php if ($ucsQuery !== ''): ?>
                <a href="<?php echo htmlspecialchars(ROOT_URL . '/admin/majors/index.php'); ?>" class="mt-4 inline-flex items-center gap-2 rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 transition-colors duration-150 hover:bg-gray-50 focus:outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600">
                    Clear search
                </a>
            <?php else: ?>
                <a href="<?php echo htmlspecialchars(ROOT_URL . '/admin/majors/create.php'); ?>" class="mt-4 inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition-colors duration-200 hover:bg-blue-700 focus:outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600">
                    Add Major
                </a>
            <?php endif; ?>
        </div>
            ...
            <table class="w-full min-w-full text-sm">
                <thead>
                    <tr class="bg-gray-50">
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Short Name</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Degree</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    <?php foreach ($ucsMajors as $ucsMajor): ?>
                        <?php
                        $ucsMajorName = (string) $ucsMajor['name'];
                        $ucsJsName    = str_replace(['\\', "'"], ['\\\\', "\\'"], $ucsMajorName);
                        ?>
                        <tr class="transition-colors hover:bg-gray-50/60">
                            <td class="whitespace-nowrap px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        <p class="font-semibold text-gray-900"><?php echo htmlspecialchars($ucsMajorName); ?></p>
                                        <p class="text-xs text-gray-500">Added <?php echo date('j M Y', strtotime((string) $ucsMajor['created_at'])); ?></p>
                                    </div>
                                </div>
                            </td>
                            <td class="whitespace-nowrap px-6 py-4 text-gray-600">
                                <?php echo !empty($ucsMajor['short_name']) ? htmlspecialchars((string) $ucsMajor['short_name']) : '<span class="text-gray-400">—</span>'; ?>
                            </td>
                            <td class="whitespace-nowrap px-6 py-4 text-gray-600">
                                <?php echo !empty($ucsMajor['degree_name']) ? htmlspecialchars((string) $ucsMajor['degree_name']) : '<span class="text-gray-400">—</span>'; ?>
                            </td>
                            <td class="max-w-md px-6 py-4 text-gray-600">
                                <?php
                                $ucsDescription = $ucsPreview($ucsMajor['description']);
                                echo $ucsDescription !== '' ? htmlspecialchars($ucsDescription) : '<span class="text-gray-400">—</span>';
                                ?>
                            </td>
                            <td class="whitespace-nowrap px-6 py-4">
                                <?php if ((int) $ucsMajor['status'] === 1): ?>
                            ...
                            <td class="whitespace-nowrap px-6 py-4">
                                <div class="flex flex-wrap items-center justify-end gap-2">
                                    <a href="<?php echo htmlspecialchars(ROOT_URL . '/admin/majors/edit.php?id=' . (int) $ucsMajor['id']); ?>" title="Edit <?php echo htmlspecialchars($ucsMajorName); ?>" aria-label="Edit <?php echo htmlspecialchars($ucsMajorName); ?>"
                                    ...
                                    <?php if ((int) $ucsMajor['status'] !== 1): ?>
                                        <form method="post" action="<?php echo htmlspecialchars(ROOT_URL . '/actions/admin/major-activate.php'); ?>" class="inline-flex">
                                    ...
                                    <?php else: ?>
                                        <form method="post" action="<?php echo htmlspecialchars(ROOT_URL . '/actions/admin/major-deactivate.php'); ?>" class="inline-flex"
                                              onsubmit="return confirm('Deactivate major &quot;<?php echo htmlspecialchars($ucsJsName); ?>&quot;? It will be hidden from degree listings until reactivated.');">
                                            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(admin_csrf_token()); ?>">
                                            <input type="hidden" name="id" value="<?php echo (int) $ucsMajor['id']; ?>">
                                            <button type="submit"
                                    ...
                                    <form method="post" action="<?php echo htmlspecialchars(ROOT_URL . '/actions/admin/major-delete.php'); ?>" class="inline-flex"
                                          onsubmit="return confirm('Delete major &quot;<?php echo htmlspecialchars($ucsJsName); ?>&quot;? This permanently removes it and can only be done if it has no linked courses, classrooms or news.');">
                                        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(admin_csrf_token()); ?>">
                                        <input type="hidden" name="id" value="<?php echo (int) $ucsMajor['id']; ?>">
                                        <button type="submit" title="Delete <?php echo htmlspecialchars($ucsMajorName); ?>" aria-label="Delete <?php echo htmlspecialchars($ucsMajorName); ?>"
