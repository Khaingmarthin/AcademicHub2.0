<?php
/**
 * Admin Career Opportunities module - edit form.
 */
...
$pageSubtitle = 'Correct the details of a posting shared by an alumnus.';
...
if ($ucsOpportunity === null) {
    header('Location: ' . ROOT_URL . '/admin/opportunities/index.php');
    exit;
}
...
$ucsForm = [
    ...
    'status'          => (string) ($ucsOld['status'] ?? $ucsOpportunity['status']),
];
...
<div class="mx-auto max-w-3xl">
    <div class="rounded-2xl bg-white shadow-sm ring-1 ring-gray-100">
        <div class="border-b border-gray-100 px-6 py-5">
            <h2 class="text-base font-semibold text-gray-900">Opportunity Details</h2>
            <p class="mt-1 text-sm text-gray-500">
                Posted by <?php echo htmlspecialchars((string) $ucsOpportunity['poster_name']); ?>
                on <?php echo htmlspecialchars(date('M j, Y', strtotime((string) $ucsOpportunity['created_at']))); ?>.
            </p>
        </div>

        <form method="post" action="<?php echo htmlspecialchars(ROOT_URL . '/actions/admin/opportunity-update.php'); ?>" novalidate>
            ...
            <div class="space-y-6 px-6 py-6">
                ...
                <div class="grid gap-6 sm:grid-cols-2">
                    ...
                    <div>
                        <label for="status" class="block text-sm font-medium text-gray-700">Visibility <span class="text-red-500">*</span></label>
                        <select id="status" name="status" required
                                class="mt-2 block w-full rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 transition-colors focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-100">
                            <option value="active" <?php echo $ucsForm['status'] === 'active' ? 'selected' : ''; ?>>Active (visible publicly)</option>
                            <option value="hidden" <?php echo $ucsForm['status'] === 'hidden' ? 'selected' : ''; ?>>Hidden (removed from public page)</option>
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea id="how_to_apply" name="how_to_apply" rows="3"
                              class="mt-2 block w-full rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 transition-colors focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-100"><?php echo htmlspecialchars($ucsForm['how_to_apply']); ?></textarea>
                </div>
            </div>

            <div class="flex flex-col-reverse gap-3 border-t border-gray-100 px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
                <a href="<?php echo htmlspecialchars(ROOT_URL . '/admin/opportunities/index.php'); ?>" class="inline-flex items-center justify-center gap-2 rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 transition-colors duration-150 hover:bg-gray-50">
                    Cancel
                </a>
                <button type="submit"
