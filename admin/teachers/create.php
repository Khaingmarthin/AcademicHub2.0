<?php
/**
 * Admin Teachers module - create form.
 */
...
$pageSubtitle = 'Create a new teacher record.';
$activeNav    = 'teachers';

$ucsErrors = $_SESSION['teacher_errors'] ?? [];
unset($_SESSION['teacher_errors']);

$ucsOld = $_SESSION['teacher_old'] ?? [
    'teacher_id'     => '',
    'name'           => '',
    'email'          => '',
    'phone'          => '',
...
<div class="mx-auto max-w-2xl">
    <div class="rounded-2xl bg-white shadow-sm ring-1 ring-gray-100">
        <div class="border-b border-gray-100 px-6 py-5">
            <h2 class="text-base font-semibold text-gray-900">New Teacher</h2>
            <p class="mt-1 text-sm text-gray-500">A teacher must belong to either a faculty or an academic department (not both).</p>
        </div>

        <form method="post" action="<?php echo htmlspecialchars(ROOT_URL . '/actions/admin/teacher-create.php'); ?>" novalidate>
            ...
            <div class="space-y-6 px-6 py-6">
                <div>
                    ...
                    <input type="text" id="teacher_id" name="teacher_id" value="<?php echo htmlspecialchars($ucsOld['teacher_id'] ?? ''); ?>" placeholder="e.g. T-001" required maxlength="50"
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" value="<?php echo htmlspecialchars($ucsOld['name'] ?? ''); ?>" placeholder="e.g. Dr. Aung Aung" required maxlength="255"
                           class="mt-2 block w-full rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 placeholder:text-gray-400 transition-colors focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-100">
                </div>
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="email" id="email" name="email" value="<?php echo htmlspecialchars($ucsOld['email'] ?? ''); ?>" placeholder="teacher@ucsmtla.edu.mm"
                               class="mt-2 block w-full rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 placeholder:text-gray-400 transition-colors focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-100">
                    </div>
                    <div>
                        <label for="phone" class="block text-sm font-medium text-gray-700">Phone</label>
                        <input type="text" id="phone" name="phone" value="<?php echo htmlspecialchars($ucsOld['phone'] ?? ''); ?>" placeholder="09-123 456 789"
                               class="mt-2 block w-full rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 placeholder:text-gray-400 transition-colors focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-100">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="specialization" name="specialization" value="<?php echo htmlspecialchars($ucsOld['specialization'] ?? ''); ?>" placeholder="e.g. Artificial Intelligence"
                           class="mt-2 block w-full rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 placeholder:text-gray-400 transition-colors focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-100">
                </div>
                    ...
                    <div>
                        ...
                        <select id="department_id" name="department_id"
                            ...
                            <?php foreach ($ucsDepartments as $ucsDeptOption): ?>
                                <option value="<?php echo (int) $ucsDeptOption['id']; ?>" <?php echo (int) ($ucsOld['department_id'] ?? 0) === (int) $ucsDeptOption['id'] ? 'selected' : ''; ?>>
