 ...
 *
 * Displays faculties and departments in separate sections.
 * Shows 7 academic departments and only 4 administration departments.
 */
...
// Administration departments - only show these 4
$ucsAdminDepts = [
    'Student Affairs',
    'Library',
    'Administration',
...
<section class="mt-8" aria-labelledby="academic-depts-heading">
    <div class="flex items-center gap-3 mb-4">
        ...
        <span class="inline-flex items-center rounded-full bg-emerald-50 px-2.5 py-0.5 text-xs font-semibold text-emerald-600 ring-1 ring-emerald-200"><?php echo count($ucsFaculties); ?> faculties</span>
    </div>

    <?php if (empty($ucsFaculties)): ?>
        <p class="text-sm text-gray-400 italic">No faculties found.</p>
    <?php else: ?>
        <div class="space-y-4">
            <?php foreach ($ucsFaculties as $ucsFaculty): ?>
                <?php
                ...
                <div class="rounded-xl bg-white shadow-sm ring-1 ring-gray-100 overflow-hidden">
                    <div class="flex items-center gap-3 border-b border-gray-100 px-5 py-3.5 bg-gradient-to-r from-emerald-50 to-transparent">
                        <div class="h-6 w-1 rounded-full bg-emerald-500" aria-hidden="true"></div>
                        <h3 class="text-sm font-bold text-slate-800"><?php echo htmlspecialchars($ucsFaculty['name']); ?></h3>
                        <span class="inline-flex items-center rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-semibold text-emerald-700"><?php echo count($ucsFacultyDepts); ?> depts</span>
                    </div>
                    ...
                    <?php else: ?>
                        <div class="divide-y divide-gray-100">
                            <?php foreach ($ucsFacultyDepts as $ucsDept): ?>
                                ...
                                <div class="flex items-center justify-between gap-3 px-5 py-3 transition-colors hover:bg-gray-50/50">
                                    <div class="min-w-0 flex-1">
...
<!-- Administration Departments Section -->
<section class="mt-8" aria-labelledby="admin-depts-heading">
    <div class="flex items-center gap-3 mb-4">
        ...
        <span class="inline-flex items-center rounded-full bg-violet-50 px-2.5 py-0.5 text-xs font-semibold text-violet-600 ring-1 ring-violet-200">4 departments</span>
    </div>

    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <?php
        $ucsAdminColors = [
            'Student Affairs' => ['gradient' => 'from-blue-500 to-blue-600', 'borderColor' => '#3b82f6', 'bgColor' => 'bg-blue-50', 'textColor' => 'text-blue-600'],
            'Library'         => ['gradient' => 'from-emerald-500 to-emerald-600', 'borderColor' => '#10b981', 'bgColor' => 'bg-emerald-50', 'textColor' => 'text-emerald-600'],
            'Administration'  => ['gradient' => 'from-amber-500 to-amber-600', 'borderColor' => '#f59e0b', 'bgColor' => 'bg-amber-50', 'textColor' => 'text-amber-600'],
            'Finance'         => ['gradient' => 'from-violet-500 to-violet-600', 'borderColor' => '#8b5cf6', 'bgColor' => 'bg-violet-50', 'textColor' => 'text-violet-600'],
        ];
        $ucsAdminIcons = [
            'Student Affairs' => '<path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M22 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path>',
            'Library'         => '<path d="M4 19.5v-15A2.5 2.5 0 0 1 6.5 2H20v20H6.5a2.5 2.5 0 0 1 0-5H20"></path>',
            'Administration'  => '<rect x="2" y="7" width="20" height="14" rx="2" ry="2"></rect><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"></path>',
            'Finance'         => '<line x1="12" y1="1" x2="12" y2="23"></line><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"></path>',
        ];
        foreach ($ucsAdminDepts as $ucsAdminDept):
            $ucsColor = $ucsAdminColors[$ucsAdminDept];
            $ucsIcon  = $ucsAdminIcons[$ucsAdminDept];
        ?>
            <div class="group relative overflow-hidden rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200/60 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md" style="border-left: 4px solid <?php echo $ucsColor['borderColor']; ?>">
                <div class="flex items-start gap-3">
                    ...
                    <div class="min-w-0">
                        <h3 class="text-sm font-bold text-slate-800"><?php echo htmlspecialchars($ucsAdminDept); ?></h3>
