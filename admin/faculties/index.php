...
$pageTitle    = 'Faculties';
$pageSubtitle = 'Manage the university\'s faculties.';
$activeNav    = 'faculties';

$ucsFlash = $_SESSION['faculty_flash'] ?? null;
unset($_SESSION['faculty_flash']);
...
<div class="rounded-2xl bg-white shadow-sm ring-1 ring-gray-100">
    <div class="flex flex-col gap-4 border-b border-gray-100 p-6 lg:flex-row lg:items-center lg:justify-between">
        <div>
            <h2 class="text-base font-semibold text-gray-900">Faculty List</h2>
            <p class="mt-1 text-sm text-gray-500">Faculties are shown on the university\'s faculty listing page.</p>
        </div>

        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
            <form method="get" action="<?php echo htmlspecialchars(ROOT_URL . '/admin/faculties/index.php'); ?>" class="relative" role="search">
                ...
                <input type="search" name="q" value="<?php echo htmlspecialchars($ucsQuery); ?>" placeholder="Search faculties…" aria-label="Search faculties"
            ...
            <a href="<?php echo htmlspecialchars(ROOT_URL . '/admin/faculties/create.php'); ?>" class="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition-colors duration-200 hover:bg-blue-700 focus:outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600">
    ...
    <?php if (empty($ucsFaculties)): ?>
        <div class="p-10 text-center">
            ...
            <h3 class="mt-4 text-lg font-semibold text-gray-800">
                <?php echo $ucsQuery !== '' ? 'No matching faculties' : 'No faculties yet'; ?>
            </h3>
            <p class="mt-2 text-sm text-gray-500">
                <?php echo $ucsQuery !== '' ? 'Try a different search term.' : 'Add your first faculty to get started.'; ?>
            </p>
            <?php if ($ucsQuery !== ''): ?>
                <a href="<?php echo htmlspecialchars(ROOT_URL . '/admin/faculties/index.php'); ?>" class="mt-4 inline-flex items-center gap-2 rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 transition-colors duration-150 hover:bg-gray-50 focus:outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600">
                    Clear search
                </a>
            <?php else: ?>
    ...
    <?php else: ?>
        <div class="overflow-x-auto">
            <table class="w-full min-w-full text-sm">
                <thead>
                    <tr class="bg-gray-50">
                        <th scope="col" class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Faculty</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    <?php foreach ($ucsFaculties as $ucsFaculty): ?>
                        <?php
                        $ucsFacultyName = (string) $ucsFaculty['name'];
                        $ucsJsName      = str_replace(['\\', "'"], ['\\\\', "\\'"], $ucsFacultyName);
                        ?>
                        <tr class="transition-colors hover:bg-gray-50/60">
                            <td class="whitespace-nowrap px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        <p class="font-semibold text-gray-900"><?php echo htmlspecialchars($ucsFacultyName); ?></p>
                                        <p class="text-xs text-gray-500">Added <?php echo date('j M Y', strtotime((string) $ucsFaculty['created_at'])); ?></p>
                                    </div>
                                </div>
                            </td>
                            <td class="max-w-md px-6 py-4 text-gray-600">
                                <?php
                                $ucsDescription = $ucsPreview($ucsFaculty['description']);
                                echo $ucsDescription !== '' ? htmlspecialchars($ucsDescription) : '<span class="text-gray-400">—</span>';
                                ?>
                            </td>
                            <td class="whitespace-nowrap px-6 py-4">
                                <?php if ((int) $ucsFaculty['status'] === 1): ?>
                            ...
                            <td class="whitespace-nowrap px-6 py-4">
                                <div class="flex flex-wrap items-center justify-end gap-2">
                                    <a href="<?php echo htmlspecialchars(ROOT_URL . '/admin/faculties/edit.php?id=' . (int) $ucsFaculty['id']); ?>" title="Edit <?php echo htmlspecialchars($ucsFacultyName); ?>" aria-label="Edit <?php echo htmlspecialchars($ucsFacultyName); ?>"
                                    ...
                                    <?php if ((int) $ucsFaculty['status'] !== 1): ?>
                                        <form method="post" action="<?php echo htmlspecialchars(ROOT_URL . '/actions/admin/faculty-activate.php'); ?>" class="inline-flex">
                                            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(admin_csrf_token()); ?>">
                                            <input type="hidden" name="id" value="<?php echo (int) $ucsFaculty['id']; ?>">
                                            <button type="submit"
                                    ...
                                    <?php else: ?>
                                        <form method="post" action="<?php echo htmlspecialchars(ROOT_URL . '/actions/admin/faculty-deactivate.php'); ?>" class="inline-flex"
                                              onsubmit="return confirm('Deactivate faculty &quot;<?php echo htmlspecialchars($ucsJsName); ?>&quot;? It will be hidden from the faculty listing until reactivated.');">
                                            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(admin_csrf_token()); ?>">
                                            <input type="hidden" name="id" value="<?php echo (int) $ucsFaculty['id']; ?>">
                                            <button type="submit"
                                    ...
                                    <form method="post" action="<?php echo htmlspecialchars(ROOT_URL . '/actions/admin/faculty-delete.php'); ?>" class="inline-flex"
                                          onsubmit="return confirm('Delete faculty &quot;<?php echo htmlspecialchars($ucsJsName); ?>&quot;? This permanently removes it.');">
                                        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(admin_csrf_token()); ?>">
                                        <input type="hidden" name="id" value="<?php echo (int) $ucsFaculty['id']; ?>">
                                        <button type="submit" title="Delete <?php echo htmlspecialchars($ucsFacultyName); ?>" aria-label="Delete <?php echo htmlspecialchars($ucsFacultyName); ?>"
