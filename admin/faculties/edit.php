<?php
/**
 * Admin Faculties module - edit form.
 */
...
$pageSubtitle = 'Update a faculty record.';
...
try {
    $ucsStmt = $pdo->prepare(
        "SELECT id, name, description, status FROM faculties WHERE id = :id LIMIT 1"
    );
...
if ($ucsFaculty === null || $ucsId === false || $ucsId < 1) {
    $_SESSION['faculty_flash'] = ['type' => 'error', 'message' => 'Faculty not found.'];
    header('Location: ' . ROOT_URL . '/admin/faculties/index.php');
    exit;
}
...
$ucsOld = $_SESSION['faculty_old'] ?? null;
unset($_SESSION['faculty_old']);

$ucsForm = [
    'name'        => $ucsOld['name'] ?? $ucsFaculty['name'],
    'description' => $ucsOld['description'] ?? ($ucsFaculty['description'] ?? ''),
    'status'      => $ucsOld['status'] ?? (int) $ucsFaculty['status'],
];
...
<div class="mx-auto max-w-2xl">
    <div class="rounded-2xl bg-white shadow-sm ring-1 ring-gray-100">
        <div class="border-b border-gray-100 px-6 py-5">
            <h2 class="text-base font-semibold text-gray-900">Edit Faculty</h2>
            <p class="mt-1 text-sm text-gray-500">The description is optional and may include a short overview of the faculty.</p>
        </div>

        <form method="post" action="<?php echo htmlspecialchars(ROOT_URL . '/actions/admin/faculty-update.php'); ?>" novalidate>
            ...
            <div class="space-y-6 px-6 py-6">
                <div>
                    ...
                    <input type="text" id="name" name="name" value="<?php echo htmlspecialchars($ucsForm['name']); ?>" placeholder="e.g. Faculty of Computer Science (FCS)" required maxlength="255"
