<?php
/**
 * Admin Classrooms - Activate handler.
 */
...
try {
    $pdo->prepare("UPDATE classrooms SET status = 1 WHERE id = :id")->execute([':id' => $ucsId]);
    classroom_flash('success', 'Classroom "' . $ucsClassroom['classroom_name'] . '" is now Active.');
} catch (Throwable $e) {
    classroom_flash('error', 'Unable to activate the classroom. Please try again.');
}
