<?php
/**
 * Admin Faculties - Deactivate handler.
 */
...
try {
    $pdo->prepare("UPDATE faculties SET status = 0 WHERE id = :id")->execute([':id' => $ucsId]);
    faculty_flash('success', 'Faculty "' . $ucsFaculty['name'] . '" has been deactivated.');
} catch (Throwable $e) {
    faculty_flash('error', 'Unable to deactivate the faculty. Please try again.');
}
