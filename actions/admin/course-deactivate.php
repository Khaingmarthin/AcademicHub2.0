<?php
/**
 * Admin Courses - Deactivate handler.
 */
...
try {
    $pdo->prepare("UPDATE courses SET status = 0 WHERE id = :id")->execute([':id' => $ucsId]);
    course_flash('success', 'Course "' . $ucsCourse['course_code'] . ' - ' . $ucsCourse['course_name'] . '" has been deactivated.');
} catch (Throwable $e) {
    course_flash('error', 'Unable to deactivate the course. Please try again.');
}
