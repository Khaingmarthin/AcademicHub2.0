<?php
/**
 * Admin Facilities - Activate handler.
 */
...
try {
    $pdo->prepare("UPDATE facilities SET status = 1 WHERE id = :id")->execute([':id' => $ucsId]);
    facility_flash('success', 'Facility "' . $ucsFacility['name'] . '" is now Active.');
} catch (Throwable $e) {
    facility_flash('error', 'Unable to activate the facility. Please try again.');
}
