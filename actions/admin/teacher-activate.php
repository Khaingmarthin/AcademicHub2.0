<?php
/**
 * Admin Teachers - Activate handler.
 */
...
try {
    $pdo->prepare("UPDATE teachers SET status = 1 WHERE id = :id")->execute([':id' => $ucsId]);
    teacher_flash('success', 'Teacher "' . $ucsTeacher['name'] . '" is now Active.');
} catch (Throwable $e) {
    teacher_flash('error', 'Unable to activate the teacher. Please try again.');
}
