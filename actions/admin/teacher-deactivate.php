<?php
/**
 * Admin Teachers - Deactivate handler.
 */
...
try {
    $pdo->prepare("UPDATE teachers SET status = 0 WHERE id = :id")->execute([':id' => $ucsId]);
    teacher_flash('success', 'Teacher "' . $ucsTeacher['name'] . '" has been deactivated.');
} catch (Throwable $e) {
    teacher_flash('error', 'Unable to deactivate the teacher. Please try again.');
}
