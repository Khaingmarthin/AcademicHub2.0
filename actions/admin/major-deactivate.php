<?php
/**
 * Admin Majors - Deactivate handler.
 */
...
try {
    $pdo->prepare("UPDATE majors SET status = 0 WHERE id = :id")->execute([':id' => $ucsId]);
    major_flash('success', 'Major "' . $ucsMajor['name'] . '" has been deactivated.');
} catch (Throwable $e) {
    major_flash('error', 'Unable to deactivate the major. Please try again.');
}
