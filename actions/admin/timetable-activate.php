<?php
/**
 * Admin Timetables - Activate handler.
 */
...
try {
    $pdo->prepare("UPDATE timetables SET status = 1 WHERE id = :id")->execute([':id' => $ucsId]);
    timetable_flash('success', 'Timetable "' . $ucsTimetable['title'] . '" is now Active.');
} catch (Throwable $e) {
    timetable_flash('error', 'Unable to activate the timetable. Please try again.');
}
