<?php
/**
 * Admin Students - Activate handler.
 */
...
try {
    $pdo->prepare("UPDATE students SET status = 1 WHERE id = :id")->execute([':id' => $ucsId]);
    student_flash('success', 'Student "' . $ucsStudent['name'] . '" is now Active.');
} catch (Throwable $e) {
    student_flash('error', 'Unable to activate the student. Please try again.');
}
