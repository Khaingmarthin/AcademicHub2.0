<?php
/**
 * Admin Departments - Deactivate handler.
 */
...
try {
    $pdo->prepare("UPDATE departments SET status = 0 WHERE id = :id")->execute([':id' => $ucsId]);
    department_flash('success', 'Department "' . $ucsDepartment['name'] . '" has been deactivated.');
} catch (Throwable $e) {
    department_flash('error', 'Unable to deactivate the department. Please try again.');
}
