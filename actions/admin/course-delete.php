...
try {
    $pdo->prepare("DELETE FROM courses WHERE id = :id")->execute([':id' => $ucsId]);
    course_flash('success', 'Course "' . $ucsCourse['course_code'] . ' - ' . $ucsCourse['course_name'] . '" deleted successfully.');
} catch (Throwable $e) {
    course_flash('error', 'Unable to delete the course. Please try again.');
}
