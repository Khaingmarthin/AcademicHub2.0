<?php
/**
 * Admin Admissions - Activate handler.
 */
...
try {
    $pdo->prepare("UPDATE admissions SET status = 1 WHERE id = :id")->execute([':id' => $ucsId]);
    admission_flash('success', 'Admission "' . $ucsAdmission['title'] . '" is now Active.');
} catch (Throwable $e) {
    admission_flash('error', 'Unable to activate the admission. Please try again.');
}
